...
<script setup lang="ts">
	...
	const changeInstr = (offset: number) => {
		...
		const index = config.instrumentKeys.indexOf(currentStrokeDropdown.value.instr) + offset;
		if(index < 0 || index >= config.instrumentKeys.length) return;
		openStrokeDropdown({
			i: currentStrokeDropdown.value.i,
	...
	const mnemonicCells = function* (instrumentKey: Instrument) {
		...
		for(let i = 1; i <= length; i++)
		{
			...
			if(words[i] || span == spanMax) {
				yield { span, text, class: getStrokeClass(i - span, instrumentKey) }
				span = 0
				text = words[i]
			}
			span++
		}	
		yield { span, text, class: getStrokeClass(length - span, instrumentKey) }
	}

	const countSpan = (words : string[], index: number) => {
		let i = index + 1

<script setup lang="ts">
    ...
    const songCount = computed(() => Object.values(shareSongs.value).filter(x => x).length )

	const tuneCount = computed(() => 99 )

	const patternCount = computed(() => 99)

	const selectionCount = computed(() => { 
		const s = songCount.value, t = tuneCount.value, p = patternCount.value
		return [
	...
	const canShare = computed(() => {
		return navigator.canShare?.({url:url.value})
	});

	// TODO: redo simple sharing
	...
	const save = () => { 
		hide()
		const blob = new Blob([rawStringUncompressed.value], { type: 'application/rhythm+json' });
		FileSaver.saveAs(blob, `${props.tuneName || "tunes"}.rhythm`)
	}
</script>
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<ul class="nav nav-tabs">
							...
							<li class="nav-item"><a class="nav-link" :class="{ active: activeTab === 1 }" href="javascript:" @click="activeTab = 1">{{i18n.t("share-dialog.raw-uncompressed")}}</a></li>
						</ul>
					...
					<div>
						...
						<button v-else @click="save" class="btn btn-primary"><fa icon="file-audio" /> Download</button>
